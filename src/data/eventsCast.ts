...
export const CAST_EVENTS: Record<string, GameEvent> = {
  ...
  bond_take_drinks: {
    ...
    weight: 1.2,
    text: '練習終わり、幼馴染のタケが「ちょっと付き合えよ」とコンビニの袋を掲げた。',
    intro: [
      { text: '河川敷。タケが缶を放ってよこす。「ガキの頃さ、ここでお前がギター見せてきたんだよな」', scene: 'backstage' },
      { text: '「正直、半分は付き合いだった。…でも今は、お前の音、本気でかっけえと思ってる」 タケが珍しく素直だ。', scene: 'backstage' },
      { text: '星を見上げて笑い合う。言葉は少ないが、十年来の相棒との距離が、また少し縮まった。', scene: 'crowd' },
    ],
    choices: [
      {
        label: '「お前がいないと始まらない」と返す',
        resultText: 'タケは照れて缶を握り潰した。「うるせえ、当たり前だろ」 その夜、二人の結束は鋼になった。',
        resultScene: 'success',
        effects: { affinity: { take: 16 }, morale: 12 },
      },
      {
        label: '黙って拳を突き合わせる',
        resultText: '言葉はいらなかった。コツン、と鳴った拳の音だけで、十分すぎるほど伝わった。',
        resultScene: 'backstage',
        effects: { affinity: { take: 12 }, morale: 8, skill: 3 },
      },
    ],
  },
  bond_take_rhythm: {
    ...
    charId: 'take',
    requireMet: { charId: 'take', met: true },
    requireAffinityMin: { charId: 'take', min: 55 },
    weight: 1,
    text: 'タケが「お前のギターに、もっと食らいつくドラムを組みたい」とスタジオに残った。',
    intro: [
      { text: '深夜のスタジオ。タケがクリックを刻みながら言う。「お前の走り癖、わかってきた。合わせにいくわ」', scene: 'studio' },
      { text: '何百回と同じフレーズを往復する。呼吸が、鼓動が、少しずつ一つになっていく。', scene: 'studio' },
      { text: '夜明け、二人は同時にキメを叩き込んだ。完璧な一致。顔を見合わせ、思わず吹き出した。', scene: 'success' },
    ],
    choices: [],
    autoEffects: { affinity: { take: 12 }, skill: 12, morale: 6, note: 'タケとの一体感がグルーヴを生んだ' },
  ...
  bond_ryo_worry: {
    ...
    weight: 1.2,
    text: 'リョウが帳簿とにらめっこしながら、深刻な顔で相談してきた。',
    intro: [
      { text: '「なあ、来月の機材ローン…大丈夫かな」 リョウは今日も不安そうだ。', scene: 'backstage' },
      { text: 'バンドの良心は、いつも一番現実を見ている。誰かが見なきゃいけない役を、黙って引き受けてきた。', scene: 'backstage' },
      { text: 'ユウは気づく。こいつの心配性に、何度も救われてきたのだと。', scene: 'backstage' },
    ],
    choices: [
      {
        label: '「お前の堅実さに助けられてる」と伝える',
        resultText: 'リョウは目を丸くし、ふっと肩の力を抜いた。「…そう言ってもらえると、やってきた甲斐があるよ」',
        resultScene: 'success',
        effects: { affinity: { ryo: 16 }, morale: 10, money: 1000 },
      },
      {
        label: '一緒に資金繰りを立て直す',
        resultText: '二人で夜通し計算した。無駄を削り、現実的な計画が立つ。リョウの表情が、久しぶりに晴れた。',
        resultScene: 'backstage',
        effects: { affinity: { ryo: 12 }, money: 3000, morale: 4 },
      },
    ],
  },
  bond_ryo_lowtone: {
    ...
    charId: 'ryo',
    requireMet: { charId: 'ryo', met: true },
    requireAffinityMin: { charId: 'ryo', min: 55 },
    weight: 1,
    text: '大事なライブの本番中、機材トラブルでユウのギターが一瞬死んだ。',
    intro: [
      { text: 'ギターから音が消えた一瞬。客席がざわつく。背筋が凍る。', scene: 'livehouse' },
      { text: 'だがその刹那、リョウの低音が前に出た。地を這うベースが、空白を堂々と埋めていく。', scene: 'crowd' },
      { text: '目が合う。リョウが小さく頷いた――「任せろ」。ユウは復帰し、何事もなかったように畳み掛けた。', scene: 'success' },
    ],
    choices: [],
    autoEffects: { affinity: { ryo: 14 }, fans: 800, morale: 10, skill: 5, note: 'リョウの信頼がピンチを救った' },
    autoResultScene: 'success',
  },
};
